...
namespace MediaController
{
    ...
    List<VideoDevice^>^ Capture::GetDevices()
    {
        IMFAttributes*  pAttributes = NULL;
        IMFActivate**   ppDevices = NULL;
        UINT32          numDevices = 0;
        List<VideoDevice^>^ devices = gcnew List<VideoDevice^>();
        ...
        {
            devices->Add(gcnew VideoDevice(ppDevices[i]));
        }
    ...
    HRESULT Capture::Initialize(HWND hwndPreview, IUnknown* pUnk)
    {
        HRESULT                         hr = S_OK;
        IMFAttributes*                  pAttributes = NULL;
        ...
        hr = pFactory->CreateInstance(CLSID_MFCaptureEngine, IID_PPV_ARGS(&pEngine));
        if (FAILED(hr))
        {
            goto Exit;
        }
...
//    HRESULT Capture::OnCaptureEvent(WPARAM wParam, LPARAM lParam)
//    {
//        GUID guidType;
//        HRESULT hrStatus;
//
...
//    }
//
//    void Capture::OnPreviewStarted(HRESULT& hrStatus)
//    {
//        m_bPreviewing = SUCCEEDED(hrStatus);
//    }
//
//    void Capture::OnPreviewStopped(HRESULT& hrStatus)
//    {
//        m_bPreviewing = false;
//    }
//
//    void Capture::OnRecordStarted(HRESULT& hrStatus)
//    {
//        m_bRecording = SUCCEEDED(hrStatus);
//    }
//
//    void Capture::OnRecordStopped(HRESULT& hrStatus)
//    {
//        m_bRecording = false;
//
//        m_pFileManager->OnRecordingStopped();
//    }


    //HRESULT Capture::StartPreview()
    //{
    //    if (m_pEngine == NULL)
    //    {
    ...
    //void Capture::StopRecordLoop()
    //{
    //    m_pFileManager->StopCycleTimer();
    //}

    //void Capture::InitializeRecordLoop()
    //{
    //    m_pFileManager->StartCycleTimer();
    //}
    ...
    //    wstring fileName = m_pFileManager->GetNextFile();

    //    PCWSTR pszDestinationFile = fileName.c_str();
    ...
    //HRESULT Capture::TakePhoto(PCWSTR pszFileName)
    //{
    //    IMFCaptureSink *pSink = NULL;
